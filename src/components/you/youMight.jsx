...
function YouMight() {
  ...
  useEffect(() => {
    getProducts()
      .then((data) => {
        if (data && data.youMightAlsoLike) {
          setProducts(data.youMightAlsoLike);
        } else {
          setProducts([]);
        }
      })
  ...
  const handleCardClick = (id) => {
    navigate(`/product/${id}`); 
    window.scrollTo({ top: 0, behavior: 'smooth' }); // Page top par scroll karwane ke liye
  };
  ...
  return (
    <div className="product-container">
      <h1 style={{ display: "flex", justifyContent: "center", padding: "50px" }}>YOU MIGHT ALSO LIKE</h1>
